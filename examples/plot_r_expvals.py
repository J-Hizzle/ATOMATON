...
Get lists of expectation values r^k for k = -2, -1, 0, 1, 2, 3, 4 from integration

2) Evaluate integrals for certain combinations of n, l
...
for kk, l_val in enumerate(l_vals):
    for jj, k_val in enumerate(k_vals):
        for ii, n_val in enumerate(n_vals):
            if l_val < n_val:
                exp_vals[kk, jj, ii] = calc_r_k_expval(n_val, l_val, Z_val, k_val)
            else: 
